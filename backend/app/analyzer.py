...
_SKILL_CATEGORIES = {
    "linguagens": {"python", "java", "javascript", "typescript", "c", "c++", "c#",
    ...
    "frontend": {"react", "next.js", "vue", "angular", "svelte", "nuxt",
                 "gatsby", "remix", "html", "css"},
    "dados_ia": {"pandas", "numpy", "scikit-learn", "tensorflow", "pytorch", "keras",
    ...
    "devops_cloud": {"docker", "kubernetes", "terraform", "ansible", "jenkins",
    ...
    "banco_dados": {"postgresql", "mysql", "sqlite", "sql server", "oracle",
                    "mariadb", "mongodb", "redis", "elasticsearch", "dynamodb",
                    "cassandra", "firebase", "neo4j", "couchdb"},
...
def _generate_insights(
    ...
    if partial_skills:
        ...
        weaknesses.append(
            f"Skills parcialmente atendidas (nível inferior ao exigido): {partial_list}"
        )
    ...
    if not resume_skills:
        weaknesses.append(
            "Não foi possível identificar skills técnicas no currículo — verifique o formato do PDF"
        )
    ...
    for skill in top_missing:
        if skill in _SKILL_CATEGORIES.get("idiomas", set()) or "english" in skill:
            priority_actions.append(
                f"Informe seu nível CEFR de {skill} no currículo e comprove com certificação"
            )
        elif skill in _SKILL_CATEGORIES.get("devops_cloud", set()):
            priority_actions.append(
                f"Crie e publique um projeto com {skill} no GitHub"
            )
        else:
            priority_actions.append(
                f"Desenvolva um projeto prático com {skill} e adicione ao portfólio"
            )
    ...
    if score >= 70:
        priority_actions.append(
            "Reposicione o currículo destacando as skills em comum com a vaga no topo"
        )
    elif score < 60 and missing_skills:
        priority_actions.append(
            "Priorize as 2-3 skills mais críticas antes de se candidatar"
        )
...
def analyze(
    ...
    try:
        ...
        insights = _generate_insights(
            resume_skills, job_skills, matched_skills, missing_skills, partial_skills, score
        )
